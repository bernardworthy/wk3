...
function TwitterUser(handle, img) {
  var user = {};
...
function TwitterMessage(user, text, retweets, timestamp, favorites) {
  var tweet = {};
  ...
  tweet.user = user; //TwitterUser(handle, profilePic);

  return tweet;
}

var someTweet = TwitterMessage(
  TwitterUser('ddh', 'http://img.jpeg'),
  'Hello, class',
  23,
  '7:40AM',
  43);
...
function gmailMessage(emailAddress, cc, bcc, ) {
  var email = {};
...
//data models are just the section of code that defines an object


//this way isn't flexible
//pattern within the braces doesn't matter
function TwitterMessage(user, text, retweets, timestamp, favorites) {
  return {
    tweet.text = text;
    tweet.retweets = retweets;
...
function TwitterMessage(spec) {
  ...
  return {
    tweet.text = spec.text;
    tweet.retweets = spec.retweets;
    tweet.timestamp = spec.timestamp;
    tweet.favorites = spec.favorites;
    tweet.user = spec.user; //TwitterUser(handle, profilePic);
  };
}

TwitterMessage({ text: 'hello', handle: 'dhh', favorites: '33' });
TwitterMessage({ handle: 'cdavies', text: 'Hiya'});
...
//js can't optimize this so it's slower
